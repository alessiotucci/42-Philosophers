...
/*   Updated: 2023/11/01 11:37:32 by atucci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	take_forks(t_plato *philo, int flag)
{
	if (flag == 1)
	{
		...
		//printf("%s TAKE FORK!\n %llu 
		//check the table result: %d\tphilo->alive[%d]%s\n", RED,
...
//		check_table(philo->table), philo->alive, RESET);
	...
	//BG_RED, philo->name, BG_RESET, my_get_time(),
	//philo->time_to_die, philo->last_time_eat);
...
{
	if (check_table(philo->table) == 0)
		// && philo->philo_is_full == 0) // condition needed
	{
